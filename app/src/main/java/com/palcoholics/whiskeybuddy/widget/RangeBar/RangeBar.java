...
public class RangeBar extends View {
    ...
    private static final float DEFAULT_TICK_WIDTH_DP = 24;
    private static final float DEFAULT_BAR_WEIGHT_PX = 2;
    private static final int DEFAULT_BAR_COLOR = Color.GRAY;
    ...
    public Parcelable onSaveInstanceState() {
        ...
        bundle.putInt("TOP_INDEX", mTopIndex);
        bundle.putInt("BOTTOM_INDEX", mBottomIndex);
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (state instanceof Bundle) {
            ...
            mTopIndex = bundle.getInt("TOP_INDEX");
            mBottomIndex = bundle.getInt("BOTTOM_INDEX");
            mFirstSetTickCount = bundle.getBoolean("FIRST_SET_TICK_COUNT");

            setThumbIndices(mTopIndex, mBottomIndex);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // The RangeBar height should be as large as possible.
        ...
        // The RangeBar width should be as small as possible.
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        final float marginTop = mTopThumb.getHalfHeight();
        final float barHeight = h - 2 * marginTop;
        mBar = new Bar(ctx, marginTop, xPos, barHeight, mTickCount, mTickWidthDP, mBarWeight, mBarColor);
        ...
        final int newBottomIndex = mBar.getNearestTickIndex(mBottomThumb);

        // Call the listener.
        if (newTopIndex != mTopIndex || newBottomIndex != mBottomIndex) {

            mTopIndex = newTopIndex;
            mBottomIndex = newBottomIndex;

            if (mListener != null) {
                mListener.onIndexChangeListener(this, mTopIndex, mBottomIndex);
            }
        }

        // Create the line connecting the two thumbs.
        mConnectingLine = new ConnectingLine(ctx, xPos, mConnectingLineWeight, mConnectingLineColor);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mConnectingLine.draw(canvas, mTopThumb, mBottomThumb);

        mTopThumb.draw(canvas);
        mBottomThumb.draw(canvas);
    ...
    public void setTickCount(int tickCount) {
        ...
        if (isValidTickCount(tickCount)) {
            ...
            if (mFirstSetTickCount) {
                mTopIndex = 0;
                mBottomIndex = mTickCount - 1;

                if (mListener != null) {
                    mListener.onIndexChangeListener(this, mTopIndex, mBottomIndex);
                }
            }
            if (indexOutOfRange(mTopIndex, mBottomIndex))
            {
                mTopIndex = 0;
                mBottomIndex = mTickCount - 1;
     ...
     * 
     * @param tickWidth Float specifying the width of each tick mark in dp.
     */
     ...
     * 
     * @param bottomThumbIndex Integer specifying the index of the bottom thumb
     * @param topThumbIndex Integer specifying the index of the top thumb
     */
    public void setThumbIndices(int bottomThumbIndex, int topThumbIndex)
    {
        if (indexOutOfRange(bottomThumbIndex, topThumbIndex)) {
            ...
            mTopIndex = bottomThumbIndex;
            mBottomIndex = topThumbIndex;
            createThumbs();
    ...
    /**
     * Gets the index of the bottom-most thumb.
     * 
    ...
    /**
     * Gets the index of the top-most thumb.
     * 
    ...
    private void rangeBarInit(Context context, AttributeSet attrs)
    {
        ...
        try {
            ...
            mTickWidthDP = ta.getDimension(R.styleable.RangeBar_tickHeight, DEFAULT_TICK_WIDTH_DP);
    ...
    /**
     * Get marginTop in each of the public attribute methods.
     *
     * @return float marginTop
     */
    private float getMarginTop() {
        return ((mTopThumb != null) ? mTopThumb.getHalfWidth() : 0);
    }

    /**
     * Get xPos in each of the public attribute methods.
     *
     * @return float xPos
     */
    ...
    /**
     * Get barHeight in each of the public attribute methods.
     *
     * @return float barHeight
     */
    private float getBarHeight() {
        return (getHeight() - 2 * getMarginTop());
    }

    /**
     * Returns if either index is outside the range of the tickCount.
     * 
     * @param bottomThumbIndex Integer specifying the bottom thumb index.
     * @param topThumbIndex Integer specifying the top thumb index.
     * @return boolean If the index is out of range.
     */
    private boolean indexOutOfRange(int bottomThumbIndex, int topThumbIndex) {
        return (bottomThumbIndex < 0 || bottomThumbIndex >= mTickCount
                || topThumbIndex < 0
                || topThumbIndex >= mTickCount);
    }
    ...
    private void onActionDown(float x, float y) {
        ...
        if (!mTopThumb.isPressed() && mTopThumb.isInTargetZone(x, y)) {

            pressThumb(mTopThumb);

        } else if (!mTopThumb.isPressed() && mBottomThumb.isInTargetZone(x, y)) {

            pressThumb(mBottomThumb);
        }
    }
    ...
    private void onActionUp(float x, float y) {
        ...
        if (mTopThumb.isPressed()) {

            releaseThumb(mTopThumb);

		} else if (mBottomThumb.isPressed()) {

			releaseThumb(mBottomThumb);

		} else {

			float bottomThumbYDistance = Math.abs(mTopThumb.getY() - y);
			float topThumbYDistance = Math.abs(mBottomThumb.getY() - y);

			if (bottomThumbYDistance < topThumbYDistance) {
				mTopThumb.setY(y);
				releaseThumb(mTopThumb);
			} else {
				mBottomThumb.setY(y);
	        ...
	        final int newBottomIndex = mBar.getNearestTickIndex(mTopThumb);
	        final int newTopIndex = mBar.getNearestTickIndex(mBottomThumb);

	        // If either of the indices have changed, update and call the listener.
	        if (newBottomIndex != mTopIndex || newTopIndex != mBottomIndex) {

	            mTopIndex = newBottomIndex;
	            mBottomIndex = newTopIndex;
    ...
    private void onActionMove(float y) {
        ...
        // Move the pressed thumb to the new y-position.
        if (mTopThumb.isPressed()) {
            moveThumb(mTopThumb, y);
        } else if (mBottomThumb.isPressed()) {
            moveThumb(mBottomThumb, y);
        }

        // If the thumbs have switched order, fix the references.
        if (mTopThumb.getY() > mBottomThumb.getY()) {
            final Thumb temp = mTopThumb;
            mTopThumb = mBottomThumb;
            mBottomThumb = temp;
        }
    ...
    private void releaseThumb(Thumb thumb) {
        ...
        final float nearestTickY = mBar.getNearestTickCoordinate(thumb);
        thumb.setY(nearestTickY);
        thumb.release();
        invalidate();
    }
    ...
    private void moveThumb(Thumb thumb, float y) {
        ...
        if (y < mBar.getTopY() || y > mBar.getBottomY()) {
            // Do nothing.
        } else {
            thumb.setY(y);
            invalidate();
